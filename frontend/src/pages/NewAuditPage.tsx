...
import { createDemoAudit, createOfflineAudit, getKnowledgePacks, previewOfflineAudit, type AuditPreview } from "../api/client";
...
export function NewAuditPage() {
  ...
  const startDemo = async () => {
    ...
    catch (reason) { setError(reason instanceof Error ? reason.message : t("common.unknownError")); setWorking(false); }
  };
  const inspectFiles = async () => {
    if (!files.length) { setError(t("newAudit.fileRequired")); return; }
    setWorking(true); setError(null);
    try { setPreview(await previewOfflineAudit(files)); setStep(2); }
    catch (reason) { setError(reason instanceof Error ? reason.message : t("common.unknownError")); }
    finally { setWorking(false); }
  };
  const startUpload = form.handleSubmit(async (values) => {
    if (!contextSchema.safeParse(values).success || files.length === 0) { setError(t("newAudit.fileRequired")); return; }
    setWorking(true); setError(null);
  ...
  return <div className="content-page wizard-page"><p className="eyebrow">{t("newAudit.eyebrow")}</p><h1>{t("newAudit.title")}</h1><p className="page-intro">{t("newAudit.description")}</p><ol className="wizard-progress"><li className={step >= 1 ? "active" : ""}><span>{step > 1 ? <Check size={15} aria-hidden /> : "1"}</span>{t("newAudit.stepSource")}</li><li className={step >= 2 ? "active" : ""}><span>{step > 2 ? <Check size={15} aria-hidden /> : "2"}</span>{t("newAudit.stepBaseline")}</li><li className={step >= 3 ? "active" : ""}><span>3</span>{t("newAudit.stepContext")}</li></ol>{error ? <div className="inline-error" role="alert">{error}</div> : null}
