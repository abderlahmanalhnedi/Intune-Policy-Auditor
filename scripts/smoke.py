...
def request(url: str, method: str = "GET") -> tuple[int, bytes]:
    ...
    with urllib.request.urlopen(
        operation, timeout=REQUEST_TIMEOUT_SECONDS
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="ipa-smoke-") as data_directory:
        ...
        log_path = Path(data_directory) / "uvicorn.log"
        with log_path.open("w+", encoding="utf-8") as server_log:
            ...
            try:
                ...
                demo_status, demo_body = request(
                    f"{base}/api/v1/audits/demo?language=en", "POST"
                )
